...
CHAT_ID="ISI_CHAT_ID"
mkdir -p $BACKUP_DIR
tar -czf $BACKUP_DIR/$FILE /etc/passwd /etc/group /etc/shadow /etc/gshadow /etc/xray /etc/v2ray /etc/trojan-go /etc/ssh /etc/nginx /etc/systemd/system /etc/limit /etc/Jos-vpn-store /var/lib
